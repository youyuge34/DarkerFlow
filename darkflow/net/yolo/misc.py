...
def labels(meta, FLAGS):
    '''
    加载label标签：
    如果模型为上面coco或者VOC中的一个，则加载对应的label
    否则加载FLAGS.labels文件，默认为label.txt
    labels全部存入meta['labels']中，若最后长度为0则默认给一个VOC
    :param meta:
    :param FLAGS:
    :return:
    '''
    ...
    if model in voc_models: 
        ...
        if model in coco_models:
            ...
            print("Model is not coco/VOC, loading the", str(file), "as labels.")
